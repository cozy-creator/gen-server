...
from _helpers import ModelRegistry, load_unet, load_vae, load_text_encoder, load_text_encoder_2
...
def main():
    ...
    components['vae'] = load_vae(components["vae"], json.load(open(components['vae_config'])), device="cuda")
    components['text_encoder'] = load_text_encoder(components['text_encoder'], json.load(open(components['text_config'])), device="cuda")
    components['tokenizer'] = CLIPTokenizer.from_pretrained("openai/clip-vit-large-patch14")
    components['unet'] = load_unet(components['unet'], json.load(open(components['unet_config'])), device="cuda")
    components['scheduler'] = DDIMScheduler.from_pretrained("runwayml/stable-diffusion-v1-5", subfolder="scheduler")
    ...
    if arch_id == "sdxl":
        components['text_encoder_2'] = load_text_encoder_2(components['text_encoder_2'], json.load(open("sdxl_text2_config.json")), device="cuda", has_projection=True)
        components['tokenizer_2'] = CLIPTokenizer.from_pretrained("laion/CLIP-ViT-bigG-14-laion2B-39B-b160k")
