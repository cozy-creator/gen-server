...
func runApp(_ *cobra.Command, _ []string) error {
	...
	go func() {
		defer wg.Done()
		if err := runPythonRuntime(ctx, cfg, app); err != nil {
			errc <- err
		}
	}()
...
func runPythonRuntime(ctx context.Context, cfg *config.Config, app *app.App) error {
	if err := tools.StartPythonRuntime(ctx, cfg, app); err != nil {
		fmt.Println(err)
		return err
	}
