...
class VAEArch(Architecture[AutoencoderKL]):
    ...
    def _determine_type(metadata: dict[str, Any]) -> tuple[ComponentMetadata, str]:
        ...
        if architecture == "stable-diffusion-v3-medium":
            ...
            config_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "sdxl_config.json")
        else:
            result: ComponentMetadata = {
            ...
            config_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "sd1_config.json")
    ...
    @classmethod
    def detect(cls, state_dict: StateDict, metadata: dict[str, Any]) -> Optional[ComponentMetadata]:
        required_keys = {
