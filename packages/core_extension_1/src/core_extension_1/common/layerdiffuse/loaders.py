...
from .models import (
    ...
    AttentionSharingProcessor2_0,
    IPAdapterAttnShareProcessor2_0,
)
...
def merge_delta_weights_into_unet(pipe, delta_weights):
    ...
    for key in delta_weights.keys():
        ...
        unet_weights[key] = unet_weights[key].to(
            dtype=delta_weights[key].dtype
...
def load_lora_to_unet(unet, model_path, frames=1):
    ...
    for i in range(32):
        ...
        if isinstance(attn_module.processor, IPAdapterAttnProcessor2_0):
            u = IPAdapterAttnShareProcessor2_0(attn_module, frames=frames).to(
                unet.dtype
            )
