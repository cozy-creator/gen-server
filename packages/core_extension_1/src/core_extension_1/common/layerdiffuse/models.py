...
class TransparentVAEDecoder(AutoencoderKL):
    ...
    def decode(
        self, z: torch.Tensor, return_dict: bool = True, generator=None
        ...
        for i in range(int(z.shape[0])):
            if self.mod_number != 1 and i % self.mod_number != 0:
                img = torch.cat(
                    (pixel[i : i + 1], torch.ones_like(pixel[i : i + 1, :1, :, :])),
                    dim=1,
                )
...
class AttentionSharingProcessor(nn.Module):
    def __init__(self, module, frames=2, rank=256):
        ...
        self.to_q_lora = [
            LoRALinearLayer(q_in_channels, q_out_channels, rank)
            for _ in range(self.frames)
        ]
        self.to_k_lora = [
            LoRALinearLayer(k_in_channels, k_out_channels, rank)
            for _ in range(self.frames)
        ]
        self.to_v_lora = [
            LoRALinearLayer(v_in_channels, v_out_channels, rank)
            for _ in range(self.frames)
        ]
        self.to_out_lora = [
            LoRALinearLayer(o_in_channels, o_out_channels, rank)
            for _ in range(self.frames)
        ]
    ...
    def __call__(
        ...
        **kwargs,
        ...
        modified_hidden_states = einops.rearrange(
            hidden_states, "(b f) d c -> f b d c", f=self.frames
        )
        ...
        if encoder_hidden_states is None:
            ...
            framed_context = einops.rearrange(
                encoder_hidden_states, "(b f) d c -> f b d c", f=self.frames
            )
    ...
    def __call__(
        ...
        **kwargs,
        ...
        for f in range(self.frames):
            ...
            query = query.view(frame_batch_size, -1, attn.heads, head_dim).transpose(
                1, 2
            )
            key = key.view(frame_batch_size, -1, attn.heads, head_dim).transpose(1, 2)
            value = value.view(frame_batch_size, -1, attn.heads, head_dim).transpose(
                1, 2
            )
            ...
            output = output.transpose(1, 2).reshape(
                frame_batch_size, -1, attn.heads * head_dim
            )
        ...
        query = query.view(temporal_batch_size, -1, attn.heads, head_dim).transpose(
            1, 2
        )
        key = key.view(temporal_batch_size, -1, attn.heads, head_dim).transpose(1, 2)
        value = value.view(temporal_batch_size, -1, attn.heads, head_dim).transpose(
            1, 2
        )
