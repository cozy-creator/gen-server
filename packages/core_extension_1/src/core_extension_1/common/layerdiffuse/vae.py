...
def build_alpha_pyramid(color, alpha, dk=1.2):
    ...
    while True:
        ...
        current_alpha = cv2.resize(
            current_alpha, (int(W / dk), int(H / dk)), interpolation=cv2.INTER_AREA
        )[:, :, None]
    return pyramid[::-1]
...
def pad_rgb(np_rgba_hwc_uint8):
    ...
    pyramid = build_alpha_pyramid(
        color=np_rgba_hwc[..., :3], alpha=np_rgba_hwc[..., 3:]
    )

    top_c, top_a = pyramid[0]
    fg = np.sum(top_c, axis=(0, 1), keepdims=True) / np.sum(
        top_a, axis=(0, 1), keepdims=True
    ).clip(1e-8, 1e32)
...
class TransparentVAEEncoder(torch.nn.Module):
    ...
    def forward(self, sd_vae, list_of_np_rgba_hwc_uint8, use_offset=True):
        ...
        rgb_padded_bchw_01 = (
            torch.from_numpy(np.stack(list_of_np_rgb_padded, axis=0))
            .float()
            .movedim(-1, 1)
        )
        rgba_bchw_01 = (
            torch.from_numpy(np.stack(list_of_np_rgba_hwc_uint8, axis=0))
            .float()
            .movedim(-1, 1)
            / 255.0
        )
        ...
        offset_feed = torch.cat([a_bchw_01, rgb_padded_bchw_01], dim=1).to(
            device=sd_vae.device, dtype=self.dtype
        )
