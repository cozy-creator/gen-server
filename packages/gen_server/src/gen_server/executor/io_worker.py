...
def run_io_worker_sync(
    tensor_queue: multiprocessing.Queue, 
    response_conn: Connection,
    file_handler: FileHandler
):
    asyncio.run(run_io_worker(tensor_queue, response_conn, file_handler))
    ...
    try:
        while True:
            try:
                ...
                if isinstance(tensor_batch, dict) and "error" in tensor_batch:
                    response_conn.send(tensor_batch)  # Send error message to API server
                    continue
        ...
        logger.info("Sending StopIteration signal and closing connections.")
