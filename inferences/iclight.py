...
rmbg.load(
    load_state_dict_from_file(
        os.path.join(models_dir, "BriaRMBG-1.4.pth"), device=device
    )
)
...
model_path = os.path.join(models_dir, "iclight_sd15_fc.safetensors")
...
rmbg.model.to(device=device, dtype=torch.float32)
...
# t2i_pipe.enable_model_cpu_offload(device=mps_device)
...
if __name__ == "__main__":
    ...
    imagee = Image.open(os.path.join(inputs_path, "anime-girl.png"))

    input_fg = np.array(imagee)
    prompt = "an anime girl under a shade in a warm and nightly environment"
    image_width = imagee.width
    image_height = imagee.height
    num_samples = 2
    seed = int.from_bytes(os.urandom(2), "big")
    steps = 1
    a_prompt = "an anime girl under a shade in a warm and nightly environment"
    n_prompt = "lowres, bad anatomy, bad hands, cropped, worst quality, bad lighting, bad composition, bad colors, bad everything"
    # n_prompt = "bad"
    cfg = 0.5
    highres_scale = 1.5
    highres_denoise = 0.5
    ...
    with torch.inference_mode():
        ...
        out = Image.fromarray(np.uint8(results[0]))
        out.save("result.png")
