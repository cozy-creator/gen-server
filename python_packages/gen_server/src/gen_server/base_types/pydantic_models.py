...
class PipelineConfig(BaseModel):
    """
    Diffusers pipeline configuration; loaded from a config.yaml file usually
    """

    source: str
    class_name: Optional[str | tuple[str, str]] = None
    components: Optional[dict[str, "PipelineConfig"]] = None

    @field_validator("components")
    def validate_components(cls, v: Any) -> Optional[dict[str, "PipelineConfig"]]:
        if v is not None:
            return {
                key: PipelineConfig(**value) if isinstance(value, dict) else value
...
class RunCommandConfig(BaseSettings):
    ...
    model_config = SettingsConfigDict(
        case_sensitive=False, cli_parse_args=True, extra="allow"
    )
