...
class CompositeImagesNode(CustomNode):
    ...
    def _load_unet(self):
        unet = UNet2DConditionModel.from_pretrained(
            "stablediffusionapi/realistic-vision-v51", subfolder="unet"
        )
    ...
    async def __call__(
        ...
        fg_latent = (
            self.vae.encode(fg_tensor.to(self.vae.dtype)).latent_dist.mode()
            * self.vae.config.scaling_factor
        )
        bg_latent = (
            self.vae.encode(bg_tensor.to(self.vae.dtype)).latent_dist.mode()
            * self.vae.config.scaling_factor
        )
        ...
        composited_image = self.vae.decode(
            composited_latent / self.vae.config.scaling_factor
        ).sample
